# FileChunker Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement a FileChunker utility class providing file MD5 hashing (Web Worker + SparkMD5) and fixed-size chunking for chunked upload workflows.

**Architecture:** Single file `src/utils/fileChunker.ts` containing the class, types, and an inlined Worker (Blob URL). The Worker contains a minimal MD5 implementation inlined as a string, avoiding importScripts. Upload orchestration delegates actual upload to a user-provided callback.

**Tech Stack:** TypeScript, SparkMD5 (dependency), Web Worker (Blob URL), AbortSignal

**Spec:** `docs/superpowers/specs/2026-04-25-file-chunker-design.md`

---

### Task 1: Install spark-md5 dependency

**Files:**
- Modify: `package.json`

- [ ] **Step 1: Install spark-md5**

Run: `npm install spark-md5`

- [ ] **Step 2: Verify install**

Run: `npm ls spark-md5`
Expected: `spark-md5@x.x.x` listed

- [ ] **Step 3: Commit**

```bash
git add package.json package-lock.json
git commit -m "chore: add spark-md5 dependency for file chunking"
```

---

### Task 2: Create FileChunker with types, constructor, and slice()

**Files:**
- Create: `src/utils/fileChunker.ts`

- [ ] **Step 1: Create fileChunker.ts with types and slice()**
...
// Minimal SparkMD5 ArrayBuffer logic inlined for Worker context.
// SparkMD5.ArrayBuffer exposes append(chunk) and end(raw) for incremental hashing.
const WORKER_CODE = `
...
// Convert a Uint8Array (64 bytes) to 16 x 32-bit words for md5cycle
...
// SparkMD5 ArrayBuffer adapter - processes in 64-byte blocks
...
SparkArrayBuffer.prototype.append = function(arr) {
  ...
  // Keep the remaining partial block
...
SparkArrayBuffer.prototype.end = function(raw) {
  ...
  var length = buff.length;
  var i;
  // Add 0x80 padding marker after the remaining data
  var tail = new Uint8Array(length < 56 ? 64 : 128);
  for (i = 0; i < length; i++) {
    tail[i] = buff[i];
  }
  tail[i] = 0x80;
  // Encode bit length using toString(16) for precision with large files
...
export class FileChunker {
  ...
  async hash(onProgress?: (percent: number) => void, signal?: AbortSignal): Promise<string> {
    ...
    // Empty file: return well-known MD5
...
Expected: No errors related to fileChunker.ts

- [ ] **Step 3: Commit**

```bash
git add src/utils/fileChunker.ts
git commit -m "feat: add FileChunker utility with hash, slice, upload, and Worker"
```
...
- [ ] **Step 1: Add FileChunker exports**
...
git commit -m "feat: export FileChunker and related types from package entry"
```
...
- [ ] **Step 2: Verify dist output contains FileChunker**

Run: `grep -c "FileChunker" dist/index.mjs`
Expected: Non-zero count, confirming FileChunker is in the bundle
